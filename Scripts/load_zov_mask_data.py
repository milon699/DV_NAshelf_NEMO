...
ds = xr.open_mfdataset(vortex_path + fname_prefix + '*_vozocrtx' + fname_suffix)
...
ds_zov_masks = {}
for i in range(len(masks)):
    ds_zov_masks[masks[i]] = ds['vozocrtx'].where(np.isnan(ds_masks[masks[i]]) != True, drop = True)
    ds_zov_masks[masks[i]].name = 'vozocrtx'

    ds_zov_masks[masks[i]] = ds_zov_masks[masks[i]].where(ds_zov_masks[masks[i]] != 0)
    #Form merged Dataset
    #if i == 0:
    #    ds_zov_tot = ds_zov_masks[masks[i]]
    #else:
    #    ds_zov_tot = xr.merge([ds_zov_tot, ds_mev_masks[masks[i]]])
    
#Save temperature data of every mask in separate netCDF file for later use
for i in tqdm(range(len(masks))):
    ds_zov_masks[masks[i]].to_netcdf('/mnt/data/ZoVelo_Masks/zov_' + masks[i] + '.nc')
